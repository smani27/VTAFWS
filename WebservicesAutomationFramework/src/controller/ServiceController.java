...
public class ServiceController {
	...
	public Hashtable sendRequest()
	{
		...
		try{
			...
			{
				ApplicationGUIMain.statusLabel.setForeground(Color.red);
				ApplicationGUIMain.statusLabel.setText("Failed : HTTP error code : "+response.getStatusLine().getStatusCode());
				throw new RuntimeException("Failed : HTTP error code : "+ response.getStatusLine().getStatusCode());
			}else{
				
				 System.setProperty("darkGREEN", "#228B22");
				 Color myColor = Color.getColor("darkGREEN");
				 ApplicationGUIMain.statusLabel.setForeground(myColor);
...
//				xmlResponse=Global.Common.replaceLast(xmlResponse, "</array>", "</Response>");
		...
		{
			ApplicationGUIMain.statusLabel.setForeground(Color.red);
			ApplicationGUIMain.statusLabel.setText(e.getMessage());
			e.printStackTrace();
		}
